...
// --- ASSETS (XP ICONS) ---
// Using a stable GitHub repository for XP icons to ensure authenticity
const ICONS = {
...
const PORTFOLIO_DATA = {
  ...
  about: {
    bio: [
      ...
      "He has completed a graduate certificate in Business Information Technology from Seneca College and a bachelor’s degree in Computing from London Metropolitan University.",
  ...
  certifications: [
    ...
    "CELPIP – English Proficiency (Level 10)",
...
// --- TYPES ---
type WindowId = 'about' | 'resume' | 'contact' | 'skills' | 'certifications' | 'testimonials' | 'computer';
...
// --- HOOKS ---
const useDraggable = (id: WindowId, initialPos: { x: number; y: number }, onFocus: () => void, isMaximized: boolean) => {
  const [pos, setPos] = useState(initialPos);
  const [dragging, setDragging] = useState(false);
  const [rel, setRel] = useState({ x: 0, y: 0 });

  const onMouseDown = (e: React.MouseEvent) => {
    if (e.button !== 0 || isMaximized) return; // Disable drag when maximized
    onFocus();
    setDragging(true);
    setRel({
      x: e.clientX - pos.x,
...
// 1. Desktop Icon
const DesktopIcon = ({ label, iconSrc, onClick }: { label: string, iconSrc: string, onClick: () => void }) => (
  <div 
...
const WindowFrame = ({ 
  ...
  const frameStyle = windowState.isMaximized 
    ? { top: 0, left: 0, width: '100%', height: 'calc(100% - 32px)', transform: 'none' } // 32px is taskbar height
  ...
  return (
    ...
    >
      ...
      {/* Content Area - Removed "File Edit View" strip as requested */}
...
const AboutContent = () => (
  <div className="bg-white min-h-full">
    {/* IE Address Bar - Functional-ish look */}
        ...
        <div key={i} className="bg-white p-3 border border-gray-400 shadow-sm flex items-center gap-4 hover:bg-[#F7F7F7] transition-colors">
           ...
           <div className="flex-1">
             ...
             <div className="h-5 bg-[#E6E6E6] border border-[#828790] relative shadow-inner">
                ...
                {/* XP segmented progress bar look */}
  ...
  <div className="bg-white min-h-full flex flex-col font-sans text-sm">
    ...
    <div className="flex-1 p-4 bg-white relative">
      <div className="absolute top-4 right-4 bg-[#FFFFE1] border border-[#F2C05F] p-3 shadow-md max-w-xs transform rotate-1">
        ...
        <p className="text-xs mt-1">📞 {PORTFOLIO_DATA.profile.phone}</p>
...
const CertificationsContent = () => (
  <div className="bg-white min-h-full flex flex-col">
    {/* Sidebar + Content Layout like XP Explorer */}
      ...
      <div className="w-full sm:w-48 bg-[#6A78B4] p-3 hidden sm:flex flex-col gap-3 bg-gradient-to-b from-[#7A8BC8] to-[#6373B0]">
        <div className="bg-white rounded-t-lg overflow-hidden shadow-sm">
           ...
           <div className="p-3 text-[11px] text-[#00156E] space-y-1 bg-[#D3E5FA]">
             <div className="hover:underline cursor-pointer flex items-center gap-1"><span className="text-gray-400">📄</span> View as slide show</div>
             <div className="hover:underline cursor-pointer flex items-center gap-1"><span className="text-gray-400">🖨️</span> Print this picture</div>
           </div>
        </div>
        ...
        <div className="bg-white rounded-t-lg overflow-hidden shadow-sm">
           ...
           <div className="p-3 text-[11px] text-[#00156E] space-y-1 bg-[#D3E5FA]">
             ...
             <div className="hover:underline cursor-pointer flex items-center gap-1"><span className="text-gray-400">🌐</span> Publish to the Web</div>
           </div>
        </div>
      </div>
...
const App = () => {
  ...
  const openWindow = (id: WindowId, title: string, icon: string) => {
    setStartOpen(false);
    ...
    // Smart default sizing for mobile/desktop
  ...
  return (
    <div className="w-full h-screen bg-[#008080] overflow-hidden select-none font-sans relative">
      ...
      {startOpen && (
        <div className="absolute bottom-8 left-0 w-72 sm:w-80 bg-white border-2 border-[#0054E3] rounded-t-lg shadow-2xl z-50 flex flex-col font-sans">
           ...
           <div className="flex flex-1 min-h-[300px]">
              {/* Left Column (White) - Functional Items Only */}
              ...
              {/* Right Column (Blue) - Functional System Links */}
           ...
           {/* Footer - Removed useless buttons */}
      ...
      <div className="absolute bottom-0 left-0 w-full h-8 bg-[#245DDA] flex items-center z-50 border-t-2 border-[#3E80ED]">
        ...
        {/* Quick Launch Area - Now Functional */}
